...
const int UPSTREAM_SERV_PORT = DEFAULT_LOCAL_PORT;
char* UPSTREAM_SERV_ADDR;
...
int main( int argc , char *argv[]) {
    ...
    struct sockaddr_in listen_addr, upstream_serv_addr;
      
    socklen_t listen_addr_len = fill_sockaddr_in(&listen_addr, AF_INET,
                                                 htons(LISTEN_PORT), LISTEN_ADDR);
   
    socklen_t upstream_serv_addr_len = fill_sockaddr_in(&upstream_serv_addr, AF_INET,
                                                      htons(UPSTREAM_SERV_PORT), inet_addr(UPSTREAM_SERV_ADDR));
    ...
    while(true) {
        ...
        printf("DNS requst is forwarding to upstream DNS server!\n");
        ...
        connect_to(tcp_frwd_socket, (struct sockaddr*)&upstream_serv_addr, upstream_serv_addr_len);
        printf("TCP forward socket has been connected with upstream DNS server successfully!\n");
